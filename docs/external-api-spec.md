# EaseSourcer 落地页生产系统 — 对接接口文档 v1.1
...
| `images` | array | 是 | 产品图,至少 1 个;我方服务器会下载。每个元素可为 **① URL 字符串**(旧形态,继续支持)**或 ② 对象 `{ "url", "name_cn?", "name_en?", "description?" }`**。`name_cn`/`name_en`/`description` 三个元数据字段**均可选**(用户上传图时填了就传过来,没填就省略)。**有则强烈建议带上**:它们作为"该图对应产品是什么"的**强参考**,喂给产品识别 / 文案 / 品类判断 / 配图,纠正仅凭产品名导致的偏差(我方会参考但因用户输入可能随意,会结合图片综合定名)。字符串与对象形态可混用 |
...
### 5.1 幂等与入库约定 ⚠️(重要,防止产品累积)

**交付接口是幂等的:每次调用返回的是该落地页的【当前全量】,不是增量。**

- 同一 `landingpage_id` 可能被**多次重新生成**(操作员调整目标产品数、重做素材等)。每次重生成后,`products.data` 会被**整体刷新为新的全量**(产品数可能变多或变少)。
- 产品 `id` 是**稳定业务键**,格式恒为 `{code}-{序号}`(如 `precision-cnc-machining-custom-1 … -12`);同一落地页跨多次交付,`id` 不变、`code` 不变。

**因此贵方入库 `products` 时,必须按落地页维度做"覆盖",严禁直接 append(只插不删):**

- 推荐做法二选一:
  1. **先删后插**:入库前先 `DELETE WHERE code = '<落地页code>'`,再插入本次 `products.data`;或
  2. **按业务 id upsert**:以我方 `id`(`{code}-{序号}`)为唯一键 upsert。
- ❌ 反例:每次拉取都 `INSERT` 成新行(用自增主键),不按 `code` 清旧 → 多次重生成/重复拉取后,**同一落地页产品会累积重复**(例:实际 12 个,页面却显示几十个)。

**自检**:对同一 `landingpage_id` 连续调用本接口,`products.data.length` 应**恒定**;最终落地页展示的产品数应**等于该值**。若页面数大于交付接口返回数,即贵方入库侧发生了累积。
...
---
*v1.1 — 新增 §5.1「幂等与入库约定」(交付返回全量、按 code 覆盖、严禁 append)。如对字段名/结构有调整意见,请反馈。*
